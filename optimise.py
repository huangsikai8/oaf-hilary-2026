...
def walk_forward(d, col, burn_in):
    ...
    for start in range(burn_in, n, REBALANCE):
        ...
        for params, sig in signals.items():
            series = d[col].to_numpy()[train][sig[train]]
        ...
        sig = signals[best]
        oos.extend(d[col].to_numpy()[test][sig[test]])
        dates.extend(d["session_start"].to_numpy()[test][sig[test]])
        chosen.append(best)

    return np.array(oos), pd.to_datetime(dates), chosen
...
def main():
    ...
    for burn in BURN_INS:
        ...
        for col in ("gross", "net_fees"):
            oos, dates, chosen = walk_forward(d, col, burn)
            base = d[col].to_numpy()[burn:][d["vrp"].to_numpy()[burn:] > 0]
            s = sharpe(oos, oy)
            print(f"    {col:9s} tuned {s:5.2f}  t {s * np.sqrt(oy):5.2f}  "
                  f"(n={len(oos):4d})   untuned {sharpe(base, oy):5.2f} (n={len(base):4d})")
